...
- Panel validation flags unresolved selections or unsafe catalog entries with inline error messages to guide editors toward valid Bunny Fonts URLs, family names, or slugs.
- `FontSelection` treats catalog family names containing `<` as unsafe and invalid, and skips CSS fallback tokens that contain `<` to avoid breaking out of `<style>` blocks.
